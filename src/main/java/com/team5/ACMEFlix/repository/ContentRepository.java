...
public interface ContentRepository extends JpaRepository<Content, Long> {
    ...
    @Query(value = "SELECT CONTENTS.ID, CONTENTS.CONTENT_TYPE, CONTENTS.DESCRIPTION, CONTENTS.IMAGE_URL, CONTENTS.TRAILER_URL, CONTENTS.IS_AGE_RESTRICTED, CONTENTS.RELEASE_DATE, CONTENTS.RUNTIME, CONTENTS.SPOKEN_LANGUAGE, CONTENTS.TITLE, GENRES.NAME FROM CONTENTS INNER JOIN GENRES ON CONTENTS.ID=GENRES.CONTENT_ID WHERE GENRES.NAME IN (:search)", nativeQuery = true)
    List<Content> findContentByGenre(String[] search);

    @Query(value = "SELECT CONTENTS.ID, CONTENTS.CONTENT_TYPE, CONTENTS.DESCRIPTION, CONTENTS.IMAGE_URL, CONTENTS.TRAILER_URL, CONTENTS.IS_AGE_RESTRICTED, CONTENTS.RELEASE_DATE, CONTENTS.RUNTIME, CONTENTS.SPOKEN_LANGUAGE, CONTENTS.TITLE, ACTORS.FULLNAME, ACTORS.IMAGE_URL FROM CONTENTS INNER JOIN ACTORS ON CONTENTS.ID=ACTORS.CONTENT_ID WHERE ACTORS.FULLNAME IN (:search) GROUP BY CONTENTS.ID ", nativeQuery = true)
