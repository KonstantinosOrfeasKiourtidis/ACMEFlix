...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    @Query(value = "SELECT * FROM CONTENTS INNER JOIN MOVIES ON CONTENTS.ID=MOVIES.CONTENT_ID WHERE UPPER(CONTENTS.TITLE) LIKE %?1% OR LOWER(CONTENTS.TITLE) LIKE %?1% OR CONTENTS.TITLE LIKE %?1%", nativeQuery = true)
